...
[[Back to Object Oriented Programming |Python6-Object-oriented-programming]] - 
[[Forward to SciPy|Python8-SciPy]]
...
for producing publication quality 2D plots matplotlib (MatPlotLib).  In the developers 
own words, MatPlotLib is
...
The underlying structure of MatPlotLib is very general and customizable.  Thankfully, 
...
commands implicitly refer to the current figure/subplot.  This is fine so long 
...
object oriented interface is powerful.  It is also more explicit.
...
* axes = a handle to the place in the subplot to put your curves (there are four 
  axes defining the locations of the 6 curves above.) 

*************************************************************************
Hands On Exercise : Making a Plot
*************************************************************************

Lets make a plot.  Retyping everything is not fun. Open up your favorite text 
editor to create a file "plot.py" to hold your plotting script.
...
2. Add a subplot to the figure: **ax = fig.add_subplot(111)**
...
Plot is pretty powerful, lets peruse the [http://matplotlib.sourceforge.net/ MatPlotLib 
documentation].


1. By editing plot.py and referencing the documentation try to add a label to an 
   axis or change the color.
2. If you're feeling adventurous and you've successfully added a label, try ax.legend()
...
Execute ax.semilogx(range(1,11),range(10)) within your script instead of ax.plot(x) 
and run it again to see the effect.
...
If you're changing plots after they render (ie show()) you have to tell MatPlotLib to 
...
MatPlotLib has an extensive [http://matplotlib.sourceforge.net/gallery.html, gallery] 
of plots with source code.  Take a look through those; they probably have 
something similar to what you want to do.
If you click on one of these images, you'll land on a page with the source that 
created it. Recently, I've noticed that the server has been throttling my 
traffic. If you see one you like but get a "Too many requests warning" go to the 
matplotlib github repository ( https://github.com/matplotlib/matplotlib/tree/master/examples/pylab_examples )
and look for the source code there.

My favorite, among these, is [[polar_bar.py | 
https://github.com/matplotlib/matplotlib/tree/master/examples/pylab_examples/polar_bar.py]].
...
MatPlotLib has a built in mechanism for handling keyboard/mouse events in a platform 
...
and some logic to identify events that your care about.  MatPlotLib handles the rest.  
...
* MatPlotLib has several other 
  [[supported events | http://matplotlib.sourceforge.net/api/backend_bases_api.html?highlight=canvas#matplotlib.backend_bases.FigureCanvasBase.mpl_connect ]]
...
MatPlotLib has the ability to plot images.  Check out the following code.  It plots 4 
subplots
